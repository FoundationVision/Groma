...
class LVISDet(LVISV1Dataset):
    ...
    def preprocess(self, data_item):
        ...
        labels = list(label2box.keys())
        random.shuffle(labels)
        new_bboxes = []
        if len(labels) > 3:
            labels = labels[:3]
        for label in labels:
            refexp = DEFAULT_TOKENS['boe'] + label.replace('_', ' ') + DEFAULT_TOKENS['eoe']
            ...
            conversations.append((self.conv_temp.roles[0], instruct))
            conversations.append((self.conv_temp.roles[1], answer))
            new_bboxes.extend(label2box[label])
