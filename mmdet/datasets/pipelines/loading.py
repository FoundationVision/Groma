...
class CustomFilterAnnotations:
    ...
    def __init__(self, min_size=None, max_size=None, keep_empty=True):
    ...
    def __call__(self, results):
        ...
        h = gt_bboxes[:, 3] - gt_bboxes[:, 1]
        box_area = w * h
        filter_min = box_area > self.min_size if self.min_size else [True] * len(box_area)
        filter_max = box_area < self.max_size if self.max_size else [True] * len(box_area)
        keep = filter_min & filter_max
        if not keep.any():
            if self.keep_empty:
                return None
            else:
